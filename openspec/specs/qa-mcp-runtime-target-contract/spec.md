# qa-mcp Runtime Target Contract

## Purpose

Define the provider-neutral immutable values and closed packaged schema used to
identify a declared project runtime target without granting runtime authority.
...
### Requirement: Physical configuration is frozen at resolution
The resolver MUST read the declared physical configuration once into a private,
typed consumer snapshot.  It MUST NOT serialize that snapshot in readiness,
errors or public target identity.

#### Scenario: Fixture changes after resolution
- **WHEN** the declared env fixture is changed, removed or replaced after a
  validated resolution is constructed
- **THEN** consumers use the admitted snapshot rather than rereading the
  fixture.
