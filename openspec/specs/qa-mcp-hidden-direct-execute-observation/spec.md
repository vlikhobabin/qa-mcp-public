# qa-mcp-hidden-direct-execute-observation Specification

## Purpose

Define the dormant passive boundary that binds an exact hidden direct-execute
main window to one hash-only marker and one bounded stable structural UIA
topology without acquiring UI action authority.
