...
executables or shell fragments, start the process without a shell, supply a
bounded GUI child environment with explicit user-profile, app-data and temp
directory keys, and return bounded launch metadata. For a newly spawned client,
the endpoint MUST classify readiness from process liveness and TPort liveness
before returning: it MUST report success only when the process is still alive
and the TPort is listening, and it MUST return a structured failure when the
process exits early or the TPort never becomes ready within the bounded timeout.
...
- **AND** it starts the TestClient detached without a shell using the bounded
  GUI child environment
...
- **AND** the response includes `ok: true`, spawned PID, `alive: true`,
  ...
  metadata that does not expose secret environment values.
...
#### Scenario: Host-agent version declares honest TestClient readiness support
...
- **THEN** the reported host-agent version is accepted only if it belongs to the
  ...
  support, unless an exact operator override is configured.
