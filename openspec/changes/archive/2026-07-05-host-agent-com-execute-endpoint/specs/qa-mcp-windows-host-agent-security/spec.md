...
MUST be returned as UTF-8 JSON without platform-output truncation or CP866
fallback decoding.
...
- **THEN** the host-agent runs the fixed worker with `--worker`, passes the
  ...
  without truncating or re-encoding it
...
- **THEN** the host-agent terminates the worker process group where supported
  and returns a fail-closed `timeout` response
- **AND** diagnostics are bounded and do not include the WorkerRequest body
