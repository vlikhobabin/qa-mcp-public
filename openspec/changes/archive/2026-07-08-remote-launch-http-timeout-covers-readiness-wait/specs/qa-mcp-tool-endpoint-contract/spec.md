...
client is usable. Because the host-agent launch endpoint waits synchronously for
readiness, the Python HTTP request for `/testclient/launch` MUST use a timeout
that covers the launch `timeout_seconds` readiness wait plus a bounded margin;
unrelated host-agent calls MUST keep the configured default host-agent timeout.
