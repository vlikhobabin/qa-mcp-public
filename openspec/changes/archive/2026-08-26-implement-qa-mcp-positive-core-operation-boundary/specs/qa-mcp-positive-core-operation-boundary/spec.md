...
- **WHEN** `value.referenceLabel` and `error.details.referenceLabel` receive exact built-in `Reference label 7`, another string, path/credential/connection/local-endpoint forms, wrong types or a string subclass
