...
still alive and the TPort is listening across a bounded persistence dwell, and
it MUST return a structured failure when the interactive launch context cannot
be prepared, the process exits early, the TPort drops during the dwell, or the
TPort never becomes ready within the bounded timeout.
