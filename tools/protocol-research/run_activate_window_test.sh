#!/usr/bin/env bash
# Card 96 / E3 — capture-free window-ACTIVATE live-verify: boot the native TestClient on :15381 and run
# activate_window_probe.py (replays open form -> open list -> drill a row -> window-level command that brings the
# buried fixture form back to front). Needs vanessa_client FREE: apache stopped (restarted on exit). Mirrors
# run_close_window_test.sh.
set -uo pipefail
...
CAP="${CAP:-runtime/protocol-research/captures/genuine-card96-activate-20260618/traffic}"
REF="${REF:-e1cib/app/Обработка.ФикстураПротоколаTestClient}"
OUT="$REPO/runtime/protocol-research/activate-window-test/$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
...
echo "== activate_window probe (ref=$REF) =="
PYTHONPATH=src python3 tools/protocol-research/activate_window_probe.py "$TPORT" "$CAP" "$REF" 2>&1 | tee "$OUT/log.txt"
echo "out=$OUT"
