#!/usr/bin/env bash
# Card 96 / E3 — capture-free open-card live-verify: boot the native TestClient on :15381 and run
# open_card_probe.py (replays open form -> open the catalog list -> «Изменить» the active row -> the card opens).
# Needs vanessa_client FREE: Vanessa DOWN + apache stopped (restarted on exit). Mirrors run_reference_field_test.sh.
set -uo pipefail
...
CAP="${CAP:-runtime/protocol-research/captures/genuine-card96-opencard-20260618/traffic}"
MARKER="${MARKER:-ФормаГруппы}"
OUT="$REPO/runtime/protocol-research/open-card-test/$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
...
echo "== open_card probe (marker=$MARKER) =="
PYTHONPATH=src python3 tools/protocol-research/open_card_probe.py "$TPORT" "$CAP" "$MARKER" 2>&1 | tee "$OUT/log.txt"
echo "out=$OUT"
