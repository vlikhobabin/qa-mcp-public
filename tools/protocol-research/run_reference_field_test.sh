#!/usr/bin/env bash
# Card 96 / E2 — capture-free reference selection live-verify: boot the native TestClient on :15381 and run
# set_reference_field_probe.py (replays open form -> type a catalog name into the ref field -> commit -> read-back).
# Needs vanessa_client FREE: Vanessa DOWN + apache stopped (restarted on exit). Mirrors run_answer_dialog_test.sh.
set -uo pipefail
...
CAP="${CAP:-runtime/protocol-research/captures/genuine-card96-ref-20260618/traffic}"
VALUE="${VALUE:-Пантера АО}"
OUT="$REPO/runtime/protocol-research/ref-field-test/$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
...
echo "== set_reference_field probe (value=$VALUE) =="
PYTHONPATH=src python3 tools/protocol-research/set_reference_field_probe.py "$TPORT" "$CAP" "$VALUE" 2>&1 | tee "$OUT/log.txt"
echo "out=$OUT"
