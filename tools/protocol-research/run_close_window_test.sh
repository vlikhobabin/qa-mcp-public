#!/usr/bin/env bash
# Card 96 / E3 — capture-free window-CLOSE live-verify: boot the native TestClient on :15381 and run
# close_window_probe.py (replays open form -> open list -> drill a row -> card opens -> window-level close).
# Needs vanessa_client FREE: apache stopped (restarted on exit). Mirrors run_open_card_test.sh.
set -uo pipefail
...
REF="${REF:-e1cib/data/Справочник.Контрагенты}"
OUT="$REPO/runtime/protocol-research/close-window-test/$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
...
echo "== close_window probe (ref=$REF) =="
PYTHONPATH=src python3 tools/protocol-research/close_window_probe.py "$TPORT" "$CAP" "$REF" 2>&1 | tee "$OUT/log.txt"
echo "out=$OUT"
