#!/usr/bin/env bash
# Card 90 / 86e — capture-free TABLE-CELL write live-verify: boot the native TestClient on :15381 and run
# set_table_cell_probe.py (writes into the active row's PF_TABLE_TEXT cell, reads back). Needs vanessa_client
# FREE: stops apache2 (Vanessa must already be down) and restarts it on exit.
set -uo pipefail
...
CAP="${CAP:-runtime/protocol-research/captures/genuine-card90-table-20260617/traffic-selfcontained}"
OUT="$REPO/runtime/protocol-research/table-cell-test/$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
...
echo "== set_table_cell probe =="
PROBE="${PROBE:-tools/protocol-research/set_table_cell_probe.py}"
PYTHONPATH=src python3 "$PROBE" "$TPORT" "$CAP" ${PROBE_ARG:-} 2>&1 | tee "$OUT/log.txt"
echo "out=$OUT"
